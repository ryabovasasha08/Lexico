...
internal class LocalStorageImpl @Inject constructor(
    ...
    override suspend fun <T> getData(key: String, cls: Class<T>, default: T): T =
        getValue(key, default) { jsonSerializer.fromJson(it, cls) }

    override suspend fun <T> getDataOrNull(key: String, cls: Class<T>): T? =
        getValueIfAvailable(key) { jsonSerializer.fromJson(it, cls) }

    override fun <T> observeData(key: String, cls: Class<out T>, default: T): Flow<T> = observeValue(key, default) {
        jsonSerializer.fromJson(it, cls)
    }

    override suspend fun <T> putData(key: String, value: T?, cls: Class<T>): Boolean {
        val jsonValue = value?.let { jsonSerializer.toJson(it, cls) }
        return putValue(key, jsonValue)
    }

    override suspend fun <T> updateData(key: String, default: T, cls: Class<T>, transformation: (T) -> T) = updateValue(
        key,
        default,
        transformation,
    ...
    override suspend fun <T> updateNullableData(key: String, cls: Class<T>, transformation: (T?) -> T?) = updateValueOrNull(
        key,
        transformation,
        transformAfterGet = { jsonSerializer.fromJson(it, cls) },
        transformBeforePut = { jsonSerializer.toJson(it, cls) }
    ...
    private suspend fun putValue(key: String, value: Any?): Boolean {
        ...
        return try {
            ...
            Log.e("Error putting value to local storage:", e.message ?: "", e)
            false
        }
    }
}